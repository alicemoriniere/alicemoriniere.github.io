...
x=[eval(ligne[0]) for ligne in lignes]
y=[eval(ligne[1]) for ligne in lignes]
...
L=np.array([((sum(M[i])-n)/sqrt(n)) for i in range (len(M))])

# OU L=(np.sum(M,axis=1)-n)/(sqrt(n))

X=np.linspace(min(L),max(L),500)
Y=np.array([stats.norm.pdf(x) for x in X])
...
def variation_totale(n,l):
    v=0
    for k in range(n+1):
        v+=abs(stats.poisson.pmf(k,mu)-stats.binom.pmf(k,n,l/n))
    v+=1-stats.poisson.cdf(n,mu=lamb)
    return(v)
